...
if (window.location.hostname == "p.eagate.573.jp") {
  var urls = [
    "//p.eagate.573.jp/game/gfdm/gitadora_exchain/p/eam/playdata/skill.html?gtype=gf&stype=0",
    "//p.eagate.573.jp/game/gfdm/gitadora_exchain/p/eam/playdata/skill.html?gtype=gf&stype=1",
    "//p.eagate.573.jp/game/gfdm/gitadora_exchain/p/eam/playdata/skill.html?gtype=dm&stype=0",
    "//p.eagate.573.jp/game/gfdm/gitadora_exchain/p/eam/playdata/skill.html?gtype=dm&stype=1"
  ];
  ...
  // get profile data
  var url = "//p.eagate.573.jp/game/gfdm/gitadora_exchain/p/eam/playdata/profile.html";
  $.ajax({
    url: url,
    async: false,
  ...
  $.ajax({
    ...
    complete: function() {
      if (count === 4) {
        ...
        $.ajax({
          url: "//gitadora-skill-viewer.herokuapp.com/exchain/upload",
          method: "POST",
          data: {
            version: "exchain",
            card_number: profile_data["card_number"],
          ...
          success: function(data){
            if (data.status === 0) {
              location = "http://gsv.fun/exchain/" + data.message + "/g";
            } else {
              alert(data.message);
            }
          }
