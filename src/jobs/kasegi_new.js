...
function gatherKasegiResult({ skillData, kasegiResult }) {
  ...
  const skillLevel = Math.floor(skillPoint / 250) * 250;
...
async function kasegi({ version, type }) {
  ...
  Object.keys(kasegiResult).forEach(async skillLevel => {
    ...
    const sql = `
            do $sql$
              begin
                update kasegi_new set list_hot=$$${listHotString}$$, list_other=$$${listOtherString}$$, count=$$${count}$$ where version=$$${version}$$ and type=$$${type}$$ and scope=${skillLevel};
                IF NOT FOUND THEN
                  insert into kasegi_new values ($$${version}$$,$$${type}$$,${skillLevel},$$${listHotString}$$,$$${listOtherString}$$,${count});
                END IF;
              end
            $sql$
          `;
    ...
    await pg.query(sql);
    console.log(`Update kasegi_new data successfully for ${version} ${type} ${skillLevel} `);
  });
}
